...
import ReportUserRadioButton from '../ReportUserRadioButton';
import RadioSwitch from '../CommonComponents/RadioSwitch';
import axios from 'axios';
import FormHolder from './SubComponents/FormHolder';
import {startMarkingLine, startMarkingGeoPoint,markExtent,clearMarkings,startMarkingPoint} from "../GeoProcessing/actions";
import { markExtentType,storeExtentData} from '../FilterMenuWrapper/actions';
import {fetchOperationLogs,closeForm} from "../DataStore/actions"
import {
...
import { toggleForm, getLatLong, fetchFormData, resetFormData} from './actions';
...
import {
  // functions for handling form input cesium marking
  removeEntityById,
  getMarkedExtent,
  getMarkedItem,
  stopMarkingItem
} from '../CesiumComponent/utils';
//constant for odisha police id
import { ODISHA_POLICE_FORMS_FLOATING_PANE_ID } from './constants';
import { saveInLocalStorage, retrieveFromLocalStorage } from '../CommonComponents/localstorage';
import downloader from '../CommonComponents/download';
class OdishaPoliceForm extends React.Component {
  ...
  pickLatLong(evt) {
    evt.preventDefault();
    evt.stopPropagation();
    this.state.markLineData = [];
    let eventName = evt.target.textContent;
    this.state.eventId = evt.target.id;
    const evtId = evt.target.id;

    //store the map for cesium ids here.
    let mapForCesiumIds = this.state.fMapOfCesiumIds;
    let self = this;
    if (eventName === "Mark Line") {
      //set the type to passed to the function
      const type = 'LINE';
      //set the id To be deleted
      const idToBeDeleted = mapForCesiumIds[type][evtId] ==
        undefined ? "__fake__id__" :
        mapForCesiumIds[type][evtId];
      //remove this line if it already exists
      removeEntityById([idToBeDeleted]);
      //this will hold the coordinates of the marked line
      let lineCoords = [];
      //set the event name otherwise the reducer form visualisation 
      //throws error
      evt.target.name = "Line";
      
      const value = getMarkedItem(type);
      //value will be set inState on promise resolved
      value
        .then(cesiumEntityObj => {
          //add the cesium id for the corresponding evtId to this array
          mapForCesiumIds[type][evtId] = cesiumEntityObj.cesiumLine._id;
          cesiumEntityObj.lineStringPoints.forEach(linePoint => {
            //create a array to store each point in line
            let linePointArray = [];
            //pushing long
            linePointArray.push(linePoint.coords[0]);
            //pushing lat
            linePointArray.push(linePoint.coords[1]);
            //push an array of this points coords in overall line coords
            lineCoords.push(linePointArray);
          });
          //the line marking action duplicates the last point
          //so we pop the last point
          lineCoords.pop();
          //show line in textBox
          self.props.getLatLong(evtId, lineCoords);
          //stop marking Line
          stopMarkingItem(type);
        })
        .catch(error => {
          console.log(error);
          alert("sorry, couldn't mark the extent, try again");
        });
    } else if (eventName === "Mark Area") {
      //remove this entity if it already exists
      removeEntityById([evtId]);
      //this function call will return a promise
      const value = getMarkedExtent(evt);
      //value will be set inState on promise resolved
      value
        .then(coordArray => {
          return self.props.getLatLong(evtId, coordArray)
        })
        .catch(error => {
          console.log(error);
          alert("sorry, couldn't mark the extent, try again");
    ...
    //block for mark point data;
    else if (eventName === "Mark Point") {
      //set the type to passed to the function
      const type = 'POINT';
      //set the id To be deleted
      const idToBeDeleted = mapForCesiumIds[type][evtId] ==
      ...
      //remove this entity if it already exists
      removeEntityById([idToBeDeleted]);
      //this.props.startMarkingPoint(evt);
      //this function call will return a promise
      const value = getMarkedItem(type);
      //value will be set inState on promise resolqved
      value
        .then(cesiumEntityObj => {
          //add the cesium id for the corresponding evtId to this array
          mapForCesiumIds[type][evtId] = cesiumEntityObj.cesiumPoint._id;
          //display the the coords in the text box
          self.props.getLatLong(evtId, {
            long: cesiumEntityObj.coords[0],
            lat: cesiumEntityObj.coords[1]
          });
          //stop marking the point
          stopMarkingItem(type);
        }
        )
        .catch(error => {
  ...
  deleteExistingDropDown = (evt) => {
   ...
   createLatLongField(evt){
       console.log("Inside create latlongField function ");
...
//Function to populate CheckList
  ...
  populateObservationReport(myobj) {
    ...
    console.log("This is from the populate form Object  :" + JSON.stringify(observationObj));
...
*/}
        ...
        <FormHolder
          ...
          */
          ...
          markedArea={this.props.markedArea}
          data={this.state.formdata}
          operationId={this.state.operationId}
          observationReportObj={this.props.observationReportObj}
          checkListReportobj={this.props.checkListReportobj}
          deBriefFormatObj={this.props.deBriefFormatObj}
          feedBackReportObj={this.props.feedBackReportObj}
          ...
          startMarkingGeoPoint={this.props.startMarkingGeoPoint}
          startMarkingLine={this.props.startMarkingLine}
          markExtentType={this.props.markExtentType}
    ...
    function mapStateToProps(state) {
      return {
        observationReportObj: state.getIn(['odishaPoliceForms', 'obsFormObj']),
        checkListReportobj: state.getIn(['odishaPoliceForms', 'chkFormObj']),
        deBriefFormatObj: state.getIn(['odishaPoliceForms', 'deBFormObj']),
        operationId: state.getIn(['odishaPoliceForms', 'operationId']),
        feedBackReportObj: state.getIn(['odishaPoliceForms', 'fdBFormObj']),
        geoProcessingLineData: state.getIn(['geoProcessing', 'lineData']),
        // markedArea: state.getIn(['visualization', 'markedArea']),
        markedExtent:state.getIn(["geoProcessing","analysisInputs"]),
    ...
    function mapDispatchToProps(dispatch) {
      return bindActionCreators({
          ...
          startMarkingLine,
          startMarkingPoint,
          startMarkingGeoPoint,
          fetchOperationLogs,
          markExtentType,
          //mark method for Area
          markExtent,
          getLatLong,
          clearMarkings,
          closeForm,
    ...
    export default connect(mapStateToProps, mapDispatchToProps)(OdishaPoliceForm);
